...
export const TreeParticles: React.FC<TreeParticlesProps> = ({ mode }) => {
  ...
  const { positions: treePos, colors: treeColors } = useMemo(() => {
    ...
    if (ctx) {
        ...
        ctx.fillText('20', width / 2, height / 2 - 60);
        ctx.fillText('26', width / 2, height / 2 + 60);
        ...
        for (let i = 0; i < COUNT; i++) {
            if (validPixels.length > 0) {
                ...
                // 调整缩放比例，使文字整体更小，从而允许环更小
                const scale = 0.020; 
                
                pos[i * 3] = pixel.x * scale + (Math.random() - 0.5) * 0.10; 
                pos[i * 3 + 1] = pixel.y * scale + (Math.random() - 0.5) * 0.10;
